...
import pathlib
import tarfile
import oss2 as alioss # 注需要安装依赖: uv add oss2
from datetime import datetime
...
# 备份压缩包
def backup_archive() -> pathlib.Path | None:
    try:
...
# 将备份文件上传到 alioss
def upload_to_alioss(target_path: pathlib.Path):
...
# 从 alioss 中删除对应的文件
def delete_from_alioss(target_path: pathlib.Path):
    if not is_upload_to_alioss:
        return
    try:
        target_file_name = f"{alioss_opts['path_prefix']}/{target_path.name}"
