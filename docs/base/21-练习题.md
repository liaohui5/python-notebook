...
# 需要备份的文件路径列表
backup_source_items = [
    pathlib.Path("./test_src/music-server"),
    pathlib.Path("./test_src/gitea-server"),
    pathlib.Path("./test_src/nginx-proxy-manager"),
    pathlib.Path("./test_src/webdav-server"),
]

# 保存备份文件的目标目录
backup_target_path = pathlib.Path("./test_dst/archives")
...
bakfname_ignore_suffixs = [
    # 自动排除备份的文件后缀
    ".log",
    ".DS_Store",
]
bakfname_ignore_dirs = [
    # 自动排除不需要备份的目录
    "log",
    "logs",
    "cache",
...
rclone_remotes = [
    {
        "remote_name": "aliyun-oss",
        "path_prefix": "server-backups/server_auto_backup",
    },
    # {
    #     "remote_name": "tencent-oss",
    #     "path_prefix": "server-auto-backups",
    # },
]


# 备份压缩时需要排除的目录/文件
def backup_exclude_filter(tarinfo):
    tar_path = pathlib.Path(tarinfo.path)
    file_ext = tar_path.suffix
    base_dir = tar_path.parent.name
    if tarinfo.isdir() and tar_path.name in bakfname_ignore_dirs:
        return None
    if tarinfo.isfile() and base_dir in bakfname_ignore_dirs:
        return None
    if file_ext in bakfname_ignore_suffixs:
        return None
    if tarinfo.name.endswith("@mount"):
        return None
    return tarinfo
...
def backup_archive() -> Optional[pathlib.Path]:
    try:
        target_archive = backup_target_path.joinpath(generate_bakfname())
        with tarfile.open(target_archive, "w:gz") as tar:
            for item in backup_source_items:
                if item.exists():
                    tar.add(item, arcname=item.name, filter=backup_exclude_filter)
                else:
                    print(f">> 备份目标不存在: {item}")
...
def count_target_files() -> int:
    backup_file_count = 0
    with os.scandir(backup_target_path) as entries:
...
def clear_target_files() -> None:
    ...
    if bakfname_with_order_items:  # 添加安全检查
        ...
        delete_item_path = backup_target_path.joinpath(delete_item["filename"])  # pyright: ignore
        delete_item_path.unlink(missing_ok=True)
        delete_by_rclone(delete_item_path)


# 使用 rclone 删除对应的文件
def delete_by_rclone(delete_item_path: pathlib.Path) -> None:
    for remote in rclone_remotes:
        remote_name = remote["remote_name"]
        target_file_path = f"{remote['path_prefix']}/{delete_item_path.name}"
        exec_cmd(
            remote_name,
            "删除",
            ["rclone", "delete", f"{remote_name}:{target_file_path}"],
        )


# 使用 rclone 上传备份文件
def upload_by_rclone(archive_item_path: pathlib.Path) -> None:
    for remote in rclone_remotes:
        remote_name = remote["remote_name"]
        local_path = f"{archive_item_path}"
        remote_dir = f"{remote['path_prefix']}/"  # 强制以 / 结尾
        exec_cmd(
            remote_name,
            "上传",
            ["rclone", "copy", local_path, f"{remote_name}:{remote_dir}"],
        )
...
def exec_cmd(
    ...
    result = exec_os_cmd(cmd_args)
    if result["success"]:
        print(f"[{ctx}]{action}备份文件成功: {cmd_args[2]}")
    else:
        print(f"[{ctx}]{action}备份文件失败: {result['stderr']}")


# 执行系统命令
def exec_os_cmd(cmd_args: List[str]) -> Dict[str, Union[bool, str]]:
    ...
    if not isinstance(cmd_args, list) or len(cmd_args) == 0:
        result["stderr"] = "Invalid input: cmd_args is empty"
        return result
    ...
    try:
        ...
        result["stderr"] = f"Command not found: '{cmd_args[0]}'."
    except PermissionError:
        result["stderr"] = f"Permission denied while executing: '{cmd_args[0]}'."
    except Exception as e:
        result["stderr"] = f"An unexpected error: {str(e)}"
    return result


# 初始化保存路径
def init_target_path() -> None:
    if not backup_target_path.exists():
...
def main() -> None:
    ...
    if target_archive:
        upload_by_rclone(target_archive)
