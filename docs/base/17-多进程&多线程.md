...
协程(纤程)也叫`微线程`, 它是由编码者去控制的, 并不是由操作系统去调度的, 这也是与线程/进程最明显的区别, 其主要功能是让代码异步的执行, 类似 nodejs 的异步, 但是实现方式不同
...
- [Python 异步IO](https://docs.python.org/zh-cn/3/library/asyncio.html)
